...
public class OrderListActivity extends AppCompatActivity  implements NavigationView.OnNavigationItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        blocUserEventClick();
        getOrderList(sessionManager.getId_site());
        orderList = new ArrayList<>();
        adapter = new OrderListAdapter(OrderListActivity.this,orderList);
    ...
    public void getOrderList(String id_site){
        RequestQueue queue = VolleySingleton.getInstance(this).getRequestQueue();
        OrderRequest request = new OrderRequest(getApplicationContext(), queue);
        request.getOrderList(id_site, limit, new OrderRequest.getOrderListCallback() {
            @SuppressLint("SetTextI18n")
            @Override
